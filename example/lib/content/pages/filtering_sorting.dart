...
final PatternPage filteringSortingPage = PatternPage(
  ...
  description:
      ...
      'Airtable-style compound query: a list of conditions, each with a column, '
      'a type-aware operator and a value, joined by a single And/Or. The sort '
      'builder edits an ordered list of columns, where list order is sort '
  ...
  blocks: const [
    ProseBlock(
      ...
      'is-not-empty. The value input adapts too: a text field, a number field, '
      'a date picker or a dropdown of the column\'s options, so a condition can '
      'never be expressed in a way its column can\'t answer. `filter.matches` '
      'implements each comparison correctly (numbers numerically, dates by day, '
      'select by value, text case-insensitively) and ignores an incomplete '
      'condition rather than dropping every row.',
    ),
    ProseBlock(
  ...
  dos: const [
    ...
    'Order multi-sorts by intent, primary key first, and keep the list '
