...
final PatternPage contextViewPage = PatternPage(
  ...
  description:
      ...
      'available width, so it never overflows down to a 320dp viewport.',
  hasLiveDemo: true,
  blocks: const [
    ProseBlock(
      ...
      'main content changes around it. Provide `onClose` only when the panel is '
      'genuinely dismissible; without it the header shows no close button and '
      'the panel reads as a permanent part of the workspace.',
    ),
  ],
  dos: const [
    ...
    'Put the panel\'s primary action in the footer so it stays reachable below '
        'a scrolling body.',
    'Reserve header actions for a couple of compact controls, such as a '
        'secondary or icon button.',
    'Provide onClose only when the panel is genuinely dismissible; omit it for '
        'a permanent column.',
    'Let the panel fill the width on narrow layouts rather than forcing a fixed '
        'width.',
  ],
  donts: const [
...
''',
  shots: const [
    Shot(pageId: 'context-view', size: ShotSize.desktop),
    Shot(pageId: 'context-view', size: ShotSize.phone),
  ],
  related: ['settings-view', 'focus-view'],
);
