...
/// Layout → Brand bloom.
final PatternPage brandBloomPage = PatternPage(
  id: 'brand-bloom',
  group: DocGroup.layout,
  navTitle: 'Brand bloom',
  title: 'Brand bloom',
  description:
      'The brand bloom is the soft radial glow that lifts an auth backdrop '
      'from a flat wash to a branded one. `DsBrandBloom` paints a single '
      'pool of the theme\'s `bloomColor` that fades to transparent, anchored '
      'by `alignment` and sized by `radius`. The default token is a quiet '
      'neutral, so the white-label look barely changes until a skin supplies '
      'its brand tint.',
  hasLiveDemo: false,
  blocks: const [
    ProseBlock(
      'The bloom is static decoration: excluded from semantics, with no '
      'animation to still under reduced motion. Layer it between a '
      '`DsAuthGradient` and the page content, and tune `opacity` when the '
...
''',
  related: ['auth-gradient', 'waiting-screens'],
);
