...
final PatternPage groupHierarchyPage = PatternPage(
  id: 'group-hierarchy',
  group: DocGroup.data,
  navTitle: 'Group hierarchy',
  ...
  blocks: const [
    ...
    ProseBlock(
      'Pair the hierarchy with the rest of the data surfaces: selecting a group '
      'scopes the grid, the board or a set of policies to that part of the '
      'organisation. A policy attached to a parent is understood to cover its '
      'children, which is what makes the hierarchy worth maintaining rather than '
      'a flat list of tags.',
    ),
  ],
  dos: const [
    ...
    'Show a count on each node so a parent communicates its size without being '
        'expanded.',
    'Keep the identifying label first and let it ellipsize; put counts and '
        'actions in the trailing area so narrow screens stay readable.',
    'Use selection to scope the rest of the screen, and let a parent stand in '
...
''',
  shots: const [
    Shot(pageId: 'group-hierarchy', size: ShotSize.desktop),
    Shot(pageId: 'group-hierarchy', size: ShotSize.phone),
  ],
  hasLiveDemo: true,
  related: ['policies', 'grouping', 'data-grid'],
);
