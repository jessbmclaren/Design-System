...
final PatternPage cellTypesPage = PatternPage(
  id: 'cell-types',
  group: DocGroup.data,
  navTitle: 'Cell types & editing',
  title: 'Cell types & editing',
  description:
      ...
      '`rating` a five-star row; `checkbox` a tick; `progress` an inline bar; '
      'and `link` an actionable label. You describe the shape of your data once '
      'and the grid renders it consistently everywhere.',
  blocks: const [
    ProseBlock(
  ...
  dos: const [
    ...
    'Treat the grid as controlled: apply `onCellChanged` to your own state and '
        'pass the updated rows back so edits and other views stay in sync.',
    'Leave computed or derived columns (like `progress`) non-editable so people '
...
''',
  shots: const [
    Shot(pageId: 'cell-types', size: ShotSize.desktop),
    Shot(pageId: 'cell-types', size: ShotSize.phone),
  ],
  hasLiveDemo: true,
  related: ['data-grid'],
);
