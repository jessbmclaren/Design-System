...
/// Content → Inline.
final PatternPage inlinePage = PatternPage(
  id: 'inline',
  group: DocGroup.content,
  navTitle: 'Inline',
  title: 'Inline',
  description:
      '`DsInline` styles a single run of text — bold, italic, monospace `code`, '
      'or strikethrough — so a sentence can carry emphasis without leaving the '
      'flow of copy. It deliberately never sets its own font size or colour: it '
      'inherits both from the surrounding `DefaultTextStyle`, so the same '
      'treatment reads correctly inside a heading, a body paragraph, or a caption '
      'without adjustment. Drop it in as a widget for a standalone run, or use the '
      '`DsInline.span` helper to weave several treatments into one `Text.rich` '
      'paragraph. Because it renders a plain `Text` with no state, timers, or '
      'network work, it stays crisp and predictable everywhere it appears.',
  hasLiveDemo: true,
  dos: const [
    'Use `bold` sparingly to mark the one word or phrase that carries the point.',
    'Reach for `code` when naming a field, token, endpoint, or literal value.',
...
''',
  shots: const [
    Shot(pageId: 'inline', size: ShotSize.desktop),
    Shot(pageId: 'inline', size: ShotSize.phone),
  ],
  related: const ['link', 'icon'],
);
