...
final PatternPage sparklinePage = PatternPage(
  ...
  description:
      ...
      'trend: no axes, gridlines, labels or legend. `DsSparkline` '
      'normalises its `values` to fill a compact `width` × `height` box and '
      'draws them as a 2px polyline, so it slots inline beside a metric or '
      'inside a dense table cell where a full chart would be too '
  ...
  blocks: const [
    ProseBlock(
      ...
      'Zero- and one-point inputs are handled: an empty list draws '
  ...
  dos: const [
    ...
    'Keep the box small and consistent so a column of sparklines lines up '
        'as a set.',
    ...
    'Feed enough points (roughly 7 to 30) to reveal a trend without turning into '
        'noise.',
    'Use `DsLineChart` instead when the reader must compare series or '
