...
final PatternPage accordionPage = PatternPage(
  ...
  description:
      ...
      'page collapses to a scannable list of headers, and people expand only '
      'the parts they care about. Build one with `DsAccordion` and a list of '
      ...
      'section collapses the rest, keeping focus on a single answer; set '
      '`allowMultiple: true` when sections are independent and people may want '
      'several open at once. Reach for it to tame FAQs, settings groups, and '
      'progressive detail — not to hide content people must not miss.',
  hasLiveDemo: true,
  blocks: const [
    ProseBlock(
      ...
      'single-open mode only the first such item stays open; the rest resolve '
      'closed. A collapsed body is removed from the tree entirely, so assistive '
      'technology never reads hidden content, and each header exposes its '
