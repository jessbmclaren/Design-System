...
final PatternPage dividerPage = PatternPage(
  ...
  dos: const [
    ...
    'Switch to `DsDividerAxis.vertical` for side-by-side content, and give it '
        'a bounded height to fill.',
    'Use `indent` / `endIndent` to align the rule with text past a leading '
        'icon or avatar.',
    'Set `length` when you want a short, centred rule rather than a full-width '
        'one.',
    'Lean on whitespace first; add a divider only when the grouping still '
        'reads as ambiguous.',
  ],
  donts: const [
    ...
    'Don\'t place a vertical divider in an unbounded-height parent; it has no '
        'extent to fill.',
    'Don\'t override `color` with a heavy or branded hue; the rule should stay '
        'a quiet hairline.',
    'Don\'t use a divider as a decorative flourish where spacing would do the '
        'job.',
  ],
  code: '''
...
''',
  shots: const [
    Shot(pageId: 'divider', size: ShotSize.desktop),
    Shot(pageId: 'divider', size: ShotSize.phone),
  ],
  related: const ['box', 'list'],
);
