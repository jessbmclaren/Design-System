  ...
  blocks: const [
    ...
    VariablesBlock(
      ...
      rows: [
        ...
        VariableRow(name: 'colorBrandSecondary', type: 'Color', example: '#0F766E', description: 'The brand\'s second mark, for a brand that runs two colours rather than one: the accent that carries a promotional call to action or a brand-owned highlight, distinct from colorPrimary and from the colorDanger signal even when both happen to be red. Defaults to colorPrimary, so the neutral base stays a single-brand system.'),
        VariableRow(name: 'colorBrandSecondaryTint', type: 'Color', example: '#E6F5F2', description: 'A soft wash of colorBrandSecondary: the quiet surface behind a secondary-brand badge or banner, the counterpart to brandTintColor for the second mark. Defaults to brandTintColor.'),
    ...
    VariablesBlock(
      ...
      rows: [
        ...
        VariableRow(name: 'buttonPrimaryGradient', type: 'List<Color>', example: '[]', description: 'The colour stops of a gradient fill for the primary button, painted instead of buttonPrimaryColorBackground and running from the top-left to the bottom-right corner. Empty by default, so the button stays a flat fill until a skin supplies stops. Only the resting and pending fills take them: a disabled button keeps its solid tint, because a gradient reads as available. Every stop has to clear AA against buttonPrimaryColorText, since the label crosses all of them.'),
    ...
    VariablesBlock(
      ...
      rows: [
        ...
        VariableRow(name: 'colorTextMuted', type: 'Color', example: '#717171', description: 'The quiet text tier beneath colorSecondaryText: a timestamp, a unit suffix, a caption that supports the line above it. Dimmer than colorSecondaryText but not the disabled treatment, so it still reads as live text. Defaults to colorIconMuted, the matching tier for glyphs. Reserve it for short supporting runs, never for a paragraph.'),
