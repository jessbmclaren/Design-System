...
final PatternPage meterChartPage = PatternPage(
  ...
  description:
      ...
      'segments laid out along one rounded horizontal bar — the most compact '
      'way to show a part-to-whole breakdown that reads at a glance: a budget '
      'split, storage by file type, traffic by channel. Each `DsMeterSegment` '
      'contributes its `value` and takes a width of `value / total`, so the '
      'segments always sum to the full bar and the eye compares shares '
      'directly. Colour comes from the design system, not the caller: leave '
      '`DsMeterSegment.color` null and each segment draws its hue in order from '
      'the validated categorical palette, with a crisp 2px surface-coloured gap '
      ...
      '320dp phone, renders a still frame with no timers or animation, and '
