...
Selection controls capture a choice without a text entry, and each one answers a different question. A `DsCheckbox` toggles an independent option or records a single consent, like "I agree to the terms". A group of `DsRadio` widgets sharing one `groupValue` presents a mutually exclusive set where exactly one option wins, such as a plan tier. A `DsSwitch` flips a single setting that takes effect the moment it moves, like "Email me updates". Choosing the right control tells people how many answers they may give and whether the change is instant or waits for a Save.
...
- Do not use two radios for a single yes/no question; use one checkbox or a switch instead.
