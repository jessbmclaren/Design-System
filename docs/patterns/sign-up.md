...
Ask for the fewest fields you truly need, then validate them where the user is looking. Name and surname share one `DsFormFieldGroup` row and stack on narrow screens. The work email validates live once touched: a format check surfaces "This email is invalid" and a directory check "Email already taken", both through the field's `errorText`. The password is captioned by `dsFirstUnmetPasswordRule`, which names one rule at a time so the error always says the next thing to fix, and a `DsPasswordStrength` meter beneath the field grades the value as it is typed. While the request is in flight, set `submitPending` to show a spinner and block repeat taps; the view never touches the network or a timer itself, so it renders identically in a screenshot and in production.
