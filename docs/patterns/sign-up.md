...
The sign-up view turns a first visit into an account. `DsSignUpView` frames registration as one focused card — a branded glyph, a short title and description, your own form and a single full-width primary action — so the moment of commitment stays calm and unmistakable. The scaffold owns only the surrounding layout, spacing and responsive behaviour: you supply the `form` (typically a `DsFormFieldGroup`) and own its validation, while `onSubmit` does the work and, when `null`, disables the button until the form is ready. Pass an optional `aside` and the card grows a benefits panel beside it on wide screens, stacking it beneath on phones so nothing is ever lost.

Lead with the value of creating an account, then ask for the fewest fields you truly need. Set `brandIcon` and `brandColor` so the card reads as yours the instant it appears, and reserve the `footer` for the returning-user path — an "Already have an account? Sign in" prompt. While the request is in flight, set `submitPending` to show a spinner and block repeat taps; the view never touches the network or a timer itself, so it renders identically in a screenshot and in production.
