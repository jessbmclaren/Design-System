...
The sign-up view turns a first visit into an account. `DsSignUpView` frames registration as one focused card (a branded glyph, a short title and description, your own form and a single full-width primary action) so the moment of commitment stays calm and unmistakable. The scaffold owns only the surrounding layout, spacing and responsive behaviour: you supply the `form` (typically a `DsFormFieldGroup`) and own its validation, while `onSubmit` does the work and, when `null`, disables the button until the form is ready. Pass an optional `aside` and the card grows a benefits panel beside it on wide screens and stacks it beneath on phones, so nothing is ever lost.
