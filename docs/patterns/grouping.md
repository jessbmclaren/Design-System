...
Reach for grouping when the question is "how do these records cluster, and what do the clusters total?" — vehicles by depot and status, drivers by team, costs by category. For moving records between clusters by hand, reach for the board view; for slicing which records are in scope at all, reach for filtering.
...
- Aggregate the columns whose totals matter (sum a cost, average a rate) and leave the rest; a header crowded with roll-ups stops being scannable.
