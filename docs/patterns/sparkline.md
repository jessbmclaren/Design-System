...
- Keep the box small and consistent so a column of sparklines lines up and scans as a set.
...
- Feed enough points (roughly 7–30) to reveal a trend without turning into noise.
- Reach for `DsLineChart` instead when the reader must compare series or read values off an axis.
